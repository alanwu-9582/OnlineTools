...
這是一段一般文字，可以使用 **粗體文字**、*斜體文字*，也可以同時展示一般段落的排版效果。

這是一個 [超連結](https://example.com) 的示範，也可以使用下標 H~2~O 與上標 10^2^。
...
行內 LaTeX：$\\sigma_{UTS} \\approx 3.45 \\times HB$
...
~~這是刪除線文字~~，後方也可以繼續加入一般文字。

插圖的寫法是：

\`![圖片說明](圖.png "圖片說明 =60%")\`

把圖片檔一起拖進工具後，就會依照檔名對應圖片。
`;
